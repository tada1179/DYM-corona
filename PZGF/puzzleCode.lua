...
local loadImageSprite = require ("downloadData").loadImageSprite_Boss3()
...
local sheetInfo_i = require("chara_icon")
local myImageSheet = graphics.newImageSheet( "chara_icon.png",system.DocumentsDirectory, sheetInfo_i:getSheet() )
local sheetInfo = require("chara_full")
local myImageSheetFull = graphics.newImageSheet( "chara_full.png",system.DocumentsDirectory, sheetInfo:getSheet() )
...
local sheet_light_i = {}
local sequenceData_i = {}
for a = 1,5,1 do
    sheet_light_i[a] = graphics.newImageSheet( image_sheet_i[a],system.DocumentsDirectory, sheetdata_light_i[a] )
    sequenceData_i[a] = { name="sheet", sheet=sheet_light_i, start=1, count=40, time=600, loopCount=1 }
...
local function onTouchGameoverFileScreen ( self, event )
    ...
    menu_barLight.SEtouchMovePuzzle()
...
--        menu_barLight.SEtouchPuzzleFight()
        menu_barLight.SEtouchEnamyFight()
        cancelAllTransitions()
...
local function BossSprite()
    ...
    local function thunder()
        menu_barLight.SEtouchPuzzleBossgread()
...
local function createCharacter(event)
    ...
    db = sqlite3.open( path )
    local m = 1
    for x in db:nrows("SELECT * FROM characterbattle GROUP BY battle_id;") do
        image_char[m] = {}
        character_numAll = tonumber(x.battleAll)
        mission_coin = tonumber(x.missionCoin)
        image_char[m].battle_id = x.battle_id
        image_char[m].battle = x.battle
        image_char[m].characAll = tonumber(x.characAll)

        local k  = 1
        for y in db:nrows("SELECT * FROM characterbattle WHERE battle_id = '"..image_char[m].battle_id .."';") do
            image_char[m][k] = {}
            image_char[m][k].charac_id = y.charac_id
            image_char[m][k].charac_name = y.charac_name
            image_char[m][k].charac_img = y.charac_img
            image_char[m][k].charac_element = tonumber(y.charac_element)
            image_char[m][k].charac_hp = tonumber(y.charac_hp)
            image_char[m][k].charac_def = tonumber(y.charac_def)
            image_char[m][k].charac_atk = tonumber(y.charac_atk )
            image_char[m][k].charac_spw = tonumber(y.charac_spw)
            image_char[m][k].charac_sph = tonumber(y.charac_sph)
            image_char[m][k].charac_countD = tonumber(y.charac_countD)
            image_char[m][k].charac_coin = tonumber(y.charac_coin)
            image_char[m][k].condition = tonumber(y.condition)

            image_char[m][k].charac_hp = image_char[m][k].charac_hp * image_char[m][k].condition
            k = k + 1
        end
        m = m + 1
    end
...
--    local myImageSheet = graphics.newImageSheet( "img/character/chara_icon.png", sheetInfo_i:getSheet() )
    ...
    local m = 1
    for x in db:nrows("SELECT * FROM team;") do
        datacharcter[m] = {}
        datacharcter[m].imagePicture= x.imgMini
        datacharcter[m].leader_id= x.leader_id
        datacharcter[m].skill_id= x.skill_id
        datacharcter[m].holdcharac_id = tonumber(x.holdcharac_id)
        datacharcter[m].element = tonumber(x.element)
        datacharcter[m].team_no = tonumber(x.team_no)
        datacharcter[m].def = tonumber(x.def)
        datacharcter[m].atk = tonumber(x.atk)
        datacharcter[m].hp = tonumber(x.hp)
    ...
    for i=1,rowCharac,1 do
        ...
        imgcharacIcon[i] = display.newImageRect( myImageSheet , sheetInfo_i:getFrameIndex(datacharcter[i].imagePicture) , _W*.16, _H*.106 )
...
local imgColor
local function sprite_sheet(color,pointX,pointY,numPoint)
    isGemTouchEnabled = false
    imgColor = nil
    ...
    local poiny = pointY
    myAnimationSheet[numPoint] = display.newSprite( sheet_light_i[color], sequenceData_i[color] )
    myAnimationSheet[numPoint]:setReferencePoint( display.BottomCenterReferencePoint)
    myAnimationSheet[numPoint].x = pointX



    if color == 1 then
        myAnimationSheet[numPoint].y = poiny + display.contentHeight*.05

    elseif color == 2 then
        myAnimationSheet[numPoint].y = poiny + display.contentHeight*.065

    elseif color == 3 then
        myAnimationSheet[numPoint].y = poiny + display.contentHeight*.03

    elseif color == 4 then
        myAnimationSheet[numPoint].y = poiny
    ...
    local function swapSheet()
        myAnimationSheet[numPoint]:setSequence( "sheet" )
        myAnimationSheet[numPoint]:play()

        transitionStash.newTransition  = transition.to(  myAnimationSheet[numPoint], { time=600, xScale=2, yScale=2, alpha=1,onComplete = clearmyAnimationSheet} )

    end
    TimersST.myTimer = timer.performWithDelay( 0, swapSheet )

    showTextCoin.text = string.format(myCount[numPoint])
    showTextCoin:setReferencePoint( display.TopLeftReferencePoint )
    showTextCoin.alpha = 1
    showTextCoin.x = pointX+_W * (mRandom(0.05,0.1))
    ...
    transitionStash.newTransition  = transition.to( showTextCoin, { time=200, xScale=1.5,x=showTextCoin.x +_W*(mRandom(0.1,0.15)),y=showTextCoin.y -_H*mRandom(0.08,0.1), yScale=1.5, alpha=1,onComplete = Clearlistener} )
...
local function battleIcon(randI,numcharacter,colercharacter,atk,numPoint)
    isGemTouchEnabled = false
    EFSpriteSheetPoint[tu] = {}

    local changeCharac_id = characImage[numcharacter]
    local changeCharac_idx = characImage[numcharacter].x
    local changeCharac_idy = characImage[numcharacter].y
        local IMGtransition
        ...
        local vx, vy = changeCharac_id.x - battleIconcolor.x, -180
        --all icon element in puzzle   vy = -400,transition.to (time = 300)
        battleIconcolor.isBullet = true
        battleIconcolor.isSleepingAllowed = true
        ...
        local function listenerSprite_sheet()
            if transitionStash.clearball then
                transition.cancel(transitionStash.clearball)
                transitionStash.clearball = nil
            end
            if CountCharacterInBattle >=1  then
                ...
                local pointX = math.ceil(changeCharac_id.x + image_char[battle][numcharacter].charac_spw)
                local pointY = math.ceil(changeCharac_id.y)
                sprite_sheet(colercharacter,pointX,pointY,numPoint)

                if changeCharac_id.id == numcharacter then
                    local function swapSheet()
                        changeCharac_idx = changeCharac_idx + 5
                    end
                    local function swapSheet2()
                        changeCharac_idx = changeCharac_idx - 10
                    end
                    TimersST.myTimer = timer.performWithDelay( 30, swapSheet )
                    TimersST.myTimer = timer.performWithDelay( 60, swapSheet2 )
                    TimersST.myTimer = timer.performWithDelay( 100, swapSheet )
                end
                getCharacterCoin(numcharacter)
            end
        ...
        if battleIconcolor.isSleepingAllowed == true then
            ...
            transitionStash.clearball = transition.to( battleIconcolor, { time=5, xScale=2, yScale=m-.5, alpha=0,onComplete = listenerSprite_sheet} )
        end
    menu_barLight.checkMemory()
end


function getCharacterCoin(point)
    local numcharacter =  point
    local changeCharac_id = characImage[numcharacter]
    local image_char_id = image_char[battle][numcharacter].charac_spw

    local pointX = math.ceil(changeCharac_id.x + image_char_id)
    local pointY = math.ceil(changeCharac_id.y)
    local character_id = changeCharac_id.charac_id
    local DropGard

    local function FlagPNGGET()
        textFlagImg.text = NumFlag
        display.remove(FlagPNG[battle-1])
        FlagPNG[battle-1] = nil
    ...
    local function getCoin()
        ...
        local function swapSheet()
            ...
            menu_barLight.SECoin()
        end
        local clearCoinSheet = function()
            display.remove(CoinSheet)
            CoinSheet = nil
        end
        local function countTime()
            ...
            if  TimersST.myTimercountTime then
                timer.cancel(TimersST.myTimercountTime)
                TimersST.myTimercountTime = nil
            end


--            if CountCharacterInBattle == 0 and battle < battleall then
            if CountCharacterInBattle == 0 then
                isGemTouchEnabled = false
                local copybattle = battle
                battle = battle + 1
                print("battle =",battle)

                if FlagPNG[copybattle]  then
                    local flagX = FlagPNG[copybattle].x
                    local flagY = FlagPNG[copybattle].y
                    local Nx = flagimg.x
                    local Ny = flagimg.y
                   menu_barLight.SEFlag()
                    transitionStash.newTransition = transition.to( FlagPNG[copybattle], { time=1000,delay = 200, x= Nx - flagX,y =  Ny - flagY ,alpha=0,onComplete = FlagPNGGET} )
                end
                ...
                if battle == battleall then
                    ...
                    Warning_Animation()
                elseif battle > battleall then
                    Victory_Animation_aura()
                    TimersST.myTimer = timer.performWithDelay( 1000,Victory_Animation_font)
                else
                    TimersST.myTimer = timer.performWithDelay( 1500, swapSheetBackground )
        ...
        TimersST.myTimer =  timer.performWithDelay( 100, swapSheet )
        TimersST.myTimercountTime =  timer.performWithDelay( 500, countTime )
        transitionStash.newTransition = transition.to(CoinSheet, { time=2000, alpha=0,onComplete = clearCoinSheet} )
    ...
    local function clearCharacter()
        ...
        if RandomDrop[battle] ~= 1 then
            ...
            if DropGard == 1 then
                RandomDrop[battle] = 1
                getFlag ()
                FlagPNG[battle] = display.newImageRect( "Item_Effect_flagtest.png",system.DocumentsDirectory, _W*.15, _H*.1 )
               ...
               groupGameTop:insert(FlagPNG[battle])
    ...
    changeCharac_id.hold_hp =  changeCharac_id.hold_hp  - hold_atk[numcharacter]

    if changeCharac_id.hold_hp <= 0 then
        ...
        NumCoin = NumCoin+changeCharac_id.coin

        transitionStash.clearCharacter = transition.to(TextCD[numcharacter], { time=200, alpha=0} )
        transitionStash.clearCharacter = transition.to(Enemy_HP[numcharacter], { time=200, alpha=0} )
        transitionStash.clearCharacter = transition.to(Enemy_bar[numcharacter], { time=200, alpha=0} )
        transitionStash.clearCharacter = transition.to(changeCharac_id, { time=200, alpha=0,onComplete = clearCharacter} )
    else

        local hpLine = (lineFULLHP*changeCharac_id.hold_hp)/changeCharac_id.hp
...
--        end
    ...
    --      for i = 1, gemX, 1 do --- x
    --        for j = 1, gemY, 1 do --- y
    --            print("ÃÂÃÂ ÃÂÃÂ¸ÃÂÃÂ¢ÃÂÃÂ ÃÂÃÂ¸ÃÂÃÂ¢ÃÂÃÂ ÃÂÃÂ¸ÃÂÃÂ¢ÃÂÃÂ ÃÂÃÂ¸ÃÂÃÂ¢i"..i.." j"..j.."    dest"..gemsTable[i][j].destination_y )
    --        end
    --    end
...
local function shiftGems ()
    isGemTouchEnabled = false
if CountCharacterInBattle then
   ...
   menu_barLight.SEtouchPuzzleBoom()
...
local function cleanUpGems()
    ...
    isGemTouchEnabled = false -- edited
    for i = 1, gemX, 1 do
        for j = 1, gemY, 1 do
...
function onGemTouch( self, event )	-- was pre-declared
   ...
   -- print("CountCharacterInBattle = ",CountCharacterInBattle)
    if event.phase == "began" and isGemTouchEnabled == true and CountCharacterInBattle ~=0 then
        ...
        lineY = display.newImageRect("img/background/puzzle/bar_twin_v.png",_W*.18,widhtLineY)
        lineY.x, lineY.y = self.markX, sizeLineStY

        lineX = display.newImageRect("img/background/puzzle/bar_twin_h.png", widhtLineX, _H*.12)
    ...
    elseif self.isFocus then
        ...
        if event.phase == "moved" and CountCharacterInBattle ~=0 and isGemTouchEnabled == true then
        ...
        elseif event.phase == "ended" or event.phase == "cancelled" then
            ...
            menu_barLight.SEtouchMovePuzzle()
            if self.markX == posX and self.markY == posY then
            else
                local ch_CD = 0
                for i=1,CountCharacterInBattle,1 do
...
--    if ( event.phase == "ended" or event.phase == "cancelled" ) and isGemTouchEnabled == false then
--        print("END CANCELed")
--        isGemTouchEnabled = false
--       menu_barLight.SEtouchMovePuzzle()
--        if self.markX == posX and self.markY == posY then
...
local function mission(event)
    ...
    db = sqlite3.open( path )
    local m = 1
    for x in db:nrows("SELECT * FROM mission_run;") do
        image_sheetBG = x.mission_img
        mission_name = x.mission_name
        mission_exp = x.mission_exp
        chapter_id = x.chapter_id
        chapter_name = x.chapter_name
        mission_stamina = x.mission_stamina
    end
    db:close()
end
-----------------------clear---------------------------------
...
function scene:createScene( event )
   ...
   countSlide = 0
    ...
    friend = event.params.friendPoint

    Maindiamond = menu_barLight.diamond()
    Olddiamond = Maindiamond

    groupGameLayer:insert ( background )

    local background1 = display.newImageRect( "img/background/bg_puzzle_test.png", _W, _H )
    ...
    BGAnimation = display.newImageRect( image_sheetBG , display.contentWidth, _H*.445 )
    BGAnimation:setReferencePoint( display.TopLeftReferencePoint )
    BGAnimation.x, BGAnimation.y =0,0
    ...
    lifeline_sh = display.newImageRect( "img/background/puzzle/life_short.png",fullLineHP,20) -- full 550
    lifeline_sh:setReferencePoint( display.TopLeftReferencePoint )
    pointStart = _W*.08
    ...
    lifeline = display.newImageRect( "img/background/puzzle/life_line.png", 600, 30) -- 490
