...
local function LoadTeam()
    ...
    if numberHold == nil then
        ...
        while(k <= Allcharacter) do
            holdcharac[k] = allRow.chracter[k].holdcharac_id
...
--
        local comprom = alertMassage.confrimDischarge(countCHNo,characterChoose,numCoin,user_id) -- confrimDischarge(countCharacter,characterAll,coin)
...
local function character_choose(id,user_id,countNo,targetX,targetY)--event.target.id,user_id,countNo,targetX,targetY
    ...
    print("in characterChoose[countNo]",characterChoose[countNo])
        ...
        elseif  event.phase == "ended" or event.phase == "release" then
            print("Character:"..event.target.id)
                ...
                if countCHNo <= numCharacter_up then
                    ...
                    --print("characterChoose[countCHNo]",characterChoose[countCHNo])
    ...
    for i = 1, numRow , 1 do  -- create colum
        for j = 1, numColum, 1 do --create row
            ...
            if countCharac < Allcharacter then
                ...
                listCharacter[countID].id = holdcharac[countID]
...
function scene:createScene( event )
    ...
    if params then
        ...
        txtCoin = display.newText(numCoin, display.contentWidth*.53, display.contentHeight*.763,typeFont, sizetext)
        txtCoin:setReferencePoint( display.TopLeftReferencePoint )
        txtCoin.text = string.format( numCoin )
        txtCoin:setTextColor(205, 170, 125)
        groupView:insert(txtCoin)

        txtUnit = display.newText(numUnit, display.contentWidth*.54, display.contentHeight*.79,typeFont, sizetext)
...
--    end
    ...
    local titleText = display.newImageRect( img_title, display.contentWidth/4.5, display.contentHeight/34 )
