...
local loadImageSprite = require ("downloadData").loadImageSprite_Upgrade_Animation3()
-------------------------------------------------
local character_id
local user_id
-------------------------------------------------
...
local sizetxtHPLV = FontAndSize.fontGuest
...
local function onBtnonclick(event)
   ...
   -- print("countCHNo ====== ",countCHNo)
    ...
    elseif event.target.id == "enchant" then
        ...
        mySpinner:start()
        local NumCoinnet = myNumCoin - NumCoin
        if NumCoinnet >=0 then

            local LinkURL = "http://133.242.169.252/DYM/power_up.php"
            local numberHold_character =  LinkURL.."?user_id="..user_id.."&character_id="..character_id.."&LVNext="
                    ..characterLEVELUP.LV.."&ExpNext="..ExpforUser.."&HPNext="..HPNext.."&DefNext="..DefNext.."&AtkNext="..AtkNext
            numberHold_character= numberHold_character.."&countCHNo="..countCHNo .."&coin="..NumCoinnet

            for i=1,countCHNo,1 do
                numberHold_character = numberHold_character.."&ch_id"..i.."="..characterChoose[i]
            end
            require("downloadData").hold_character(user_id)
           ...
           local numberHold = http.request(numberHold_character)
            ...
            if numberHold == nil then
                ...
                allRow  = json.decode(numberHold)  --coin
                local path = system.pathForFile("datas.db", system.DocumentsDirectory)
                db = sqlite3.open( path )
                local tablefill ="UPDATE user SET user_coin = '"..allRow.coin.."';"
                db:exec( tablefill )
...
--    local myImageSheet = graphics.newImageSheet( "img/character/chara_icon.png", sheetInfo:getSheet() )
    ...
    local btnleader = widget.newButton{
        defaultFile= frame[characterLEVELUP.FrameCharacter] ,
        overFile= frame[characterLEVELUP.FrameCharacter],
        width=screenW*.18, height=screenH*.115,
        onRelease = onBtnonclick
    }
    btnleader.id= "leader"
    ...
    local framImage = display.newImageRect(myImageSheet , sheetInfo:getFrameIndex(characterLEVELUP.ImageCharacter)  ,screenW*.18, screenH*.115)
...
--    local myImageSheet = graphics.newImageSheet( "img/character/chara_icon.png", sheetInfo:getSheet() )
    ...
    for i = 1, 5 ,1 do
        if countCHNo then
            if i <= countCHNo then
                local framCharac= display.newImageRect(myImageSheet , sheetInfo:getFrameIndex(Imageleader[i])  ,screenW*.12, screenH*.08)
                ...
                character[i] = widget.newButton{
                    defaultFile= frame[Frameleader[i]] ,
                    overFile= frame[Frameleader[i]],
...
function scene:createScene( event )
    myNumCoin = menu_barLight.user_coin()
...
function scene:enterScene( event )
    ...
    storyboard.purgeScene( "power_up_save" )
