...
function new(map_id,user_id)
    ...
    local function loadChapter()
        ...
        for count in db:urows ("SELECT COUNT(*) FROM MapSubstates WHERE map_id = '"..map_id.."';") do maxChapter = count end
        local count = 1
        local rows = {}
        
        for x in db:nrows("SELECT * FROM MapSubstates WHERE map_id = '"..map_id.."';") do 
            dataTable[count] = {}
    ...
    for i = 1, maxChapter, 1 do
        pointListY = pointListY + (screenH*.13)
            if dataTable[i].ID_status == "clear" then
                ...
                backButton.id=i
                txtbattle = display.newText(dataTable[i].chapter_name, screenW*.2, pointName+screenH*.05,native.systemFontBold, 25)
