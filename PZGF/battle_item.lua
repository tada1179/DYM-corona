...
local function onBtnRelease(event)
    ...
    if event.target.id == "back" then -- back button
        ...
        if holditem == "choose" then
            local linkstate = "http://133.242.169.252/dym/choosebattle_item.php"
            local response = http.request(linkstate.."?user_id="..user_id.."&chooseitem_id="..event.target.id)
        else
            local linkstate = "http://133.242.169.252/dym/update_item.php"
...
local function scrollViewList()
    ...
    local LinkURL = "http://133.242.169.252/dym/battle_item.php"
