...
local function createCharacter(event)
    ...
    local LinkURL = "http://133.242.169.252/DYM/character_battle.php"
    ...
    for i=1,character_numAll,1 do
        print("charac_id",image_char[i].charac_id)
...
local function battleIcon()
    local physics = require( "physics" )
    physics.start()
    local img =  "img/element/pink.png"
    local randI = math.random(1,6)
    local centerImg = (_W*.16)/2
    local pointx = (tonumber(_W*.166)*(randI-1)) + _W*.005 + centerImg
    local battleIconcolor = display.newImageRect( img , _W*.1, _H*.065 )
    battleIconcolor:setReferencePoint( display.CenterReferencePoint )
    battleIconcolor.x,battleIconcolor.y = pointx, _H*.36

end
local function fncallSprite()
...
function scene:createScene( event )
    ...
    --local FNsprite = alertMSN.sprite_BG()
    backgroundTop = display.newImageRect( "img/background/battle/WaterKingdomSUMMER.png" , display.contentWidth, display.contentHeight*.445 )
    backgroundTop:setReferencePoint( display.TopLeftReferencePoint )
    backgroundTop.x, backgroundTop.y = 0, 0
--    backgroundTop:setFillColor(135, 206, 250   )
...
--        require ("alertMassage").sprite_fight()
    ...
    local function clickSheet_data(event)
        if event.target.id == "spritebattle" then
            --        require ("alertMassage").sprite_fight()
            local numcharacter = math.random(1,character_numAll)
