...
local function damageAttackEnemy()
    ...
    local clearbattleIconcolor = function (obj)
        ...
        if BGdropPuzzle ~= nil then
            transitionStash.newTransition = transition.to( BGdropPuzzle, { time=100,x= BGdropPuzzle.x+10,y=BGdropPuzzle.y-10, alpha=1})
            transitionStash.newTransition = transition.to( BGdropPuzzle, { time=150,delay=50, xScale=1,x= BGdropPuzzle.x,y=BGdropPuzzle.y, yScale=1, alpha=1})
    ...
    for i=1,CountCharacterInBattle,1 do
        TextCD[i].text =  string.format("CD:"..characImage[i].hold_countD,characImage[i].hold_atk)
...
function cleanTimers(timerStash)
    print("cleanTimers",timerStash.myTimer)
...
local function battle_Animation(all,battleimg)
...
local function characterBattle()
    ...
    while i <= battle do
        ...
        for k=1,image_char[i].characAll,1  do
            ...
            Enemy_HP[k] = display.newImageRect( "img/background/frame/as_enemy_hp.png", lineFULLHP, 15 )
            Enemy_HP[k]:setReferencePoint( display.TopCenterReferencePoint)
            Enemy_HP[k].x, Enemy_HP[k].y = (_W/(image_char[i].characAll+1))*k,_H*.27
            ...
            Enemy_bar[k] = display.newImageRect( "img/background/frame/as_enemy_bar.png", 120, 16 )
            ...
            pointStartEnemy_HP[k] =  (Enemy_bar[k].x)-60
    ...
    if battle == 1 then
        battle_Animation(battleall,battle..battleall)
    end
...
local function Victory_Animation_aura()
    ...
    TimersST = timer.performWithDelay(0,FNVictory_aura )
...
local function BossSprite()
    ...
    local function flash()
        ...
        TimersST =  timer.performWithDelay( 1000, swapSheet )
    ...
    TimersST = timer.performWithDelay( 0, flash )
    TimersST = timer.performWithDelay( 500, thunder )
    TimersST = timer.performWithDelay( 500, groundcrack )
    TimersST = timer.performWithDelay( 3000, characterBattle )
    menu_barLight.checkMemory()
end
local function Warning_Animation()
    ...
    TimersST = timer.performWithDelay(0,FNWarning_Animation )
...
local function createCharacter(event)
    local mission_id = event.params.mission_id
...
local function sprite_sheet(characterAll,num,color,pointX,pointY,numPoint)
    print("2 .sprite_sheet CountCharacterInBattle = ",CountCharacterInBattle)
    local showTextCoin
    ...
    TimersST = timer.performWithDelay( 50, swapSheet )
...
local function battleIcon(randI,numcharacter,colercharacter,atk,numPoint)  --ตำแหน่งที่บอลออก.-ตำแหน่งที่บอลจะไปยิง.-สี,จำนวน atk
    print(" 2 . battleIcon CountCharacterInBattle = ",CountCharacterInBattle)
    local IMGtransition
    ...
    local function listenerSprite_sheet()
        print("2 CountCharacterInBattle>0 ",CountCharacterInBattle)
        if CountCharacterInBattle >=1 then
            print(" 2 CountCharacterInBattle>=1 ==============  ",CountCharacterInBattle)
            ...
            TimersST = timer.performWithDelay( 50)
            if characImage[numcharacter].id == numcharacter then
                ...
                TimersST = timer.performWithDelay( 50, swapSheet )
                TimersST = timer.performWithDelay( 100, swapSheet2 )
                TimersST = timer.performWithDelay( 150, swapSheet )
            end
        end
    end
...
local function getCharacterCoin(numcharacter,team_no,color,damage,k)     --ตำแหน่งที่บอลออก.ตำแหน่งที่บอลจะไปยิง.สี,จำนวน atk,ตัวที่
    print(" 1. getCharacterCoin CountCharacterInBattle",CountCharacterInBattle)
    battleIcon(team_no,numcharacter,color,damage,k)  --ตำแหน่งที่บอลออก.-ตำแหน่งที่บอลจะไปยิง.-สี,จำนวน atk

    print("numcharacter",numcharacter)
    print("[numcharacter].x",characImage[numcharacter].x)
    print("charac_spw",image_char[battle][numcharacter].charac_spw)
    print("---------------------------------")
    ...
    local function getCoin()
        ...
        local function countTime()
            if CountCharacterInBattle == 0 and battle < battleall then
                ...
                battle = battle + 1
                FlagPNG = nil
                TimersST = timer.performWithDelay( 1000, swapSheetBackground )
                if battle == battleall  then
                    Warning_Animation()
                else
                    battle_Animation(battleall,battle..battleall)
                    TimersST = timer.performWithDelay( 4000, characterBattle )
                end
            elseif battle == battleall and CountCharacterInBattle == 0 then
                Victory_Animation_aura()
                TimersST = timer.performWithDelay( 3000,Victory_Animation_font)
            end
        end
        TimersST =  timer.performWithDelay( 100, swapSheet )
        TimersST =  timer.performWithDelay( 2000, countTime )
    ...
    local function clearCharacter()
        ...
        if CountCharacterInBattle>1 then
            for i = numcharacter,CountCharacterInBattle,1 do
                ...
                if i < CountCharacterInBattle then
                    ...
                    print("1 else clearCharacter CountCharacterInBattle = ",CountCharacterInBattle)
        ...
        print("1.end for CountCharacterInBattle numcharacter== ",CountCharacterInBattle,numcharacter)
        RandomDrop = mRandom(1,3)
    ...
    characImage[numcharacter].hold_atk =  characImage[numcharacter].hold_atk  - damage
...
local function createBackButton(event)
    ...
    local flagimg = display.newImageRect( "img/background/puzzle/FLAG.png", _W*.05, _H*.04 )
...
function cancelAllTimers()
    local k, v
    print("count",#TimersST)
    for k,v in pairs(TimersST) do
        print("time v = ",v)
        print("time k = ",k)
        timer.cancel(TimersST )
        v = nil; k = nil
    end
...
local function ClearNumber()
    ...
    TimersST = timer.performWithDelay(1000,damageAttackEnemy)
    enableGemTouch()
...
local function shiftGems ()
    ...
    TimersST = timer.performWithDelay( 0, checkdoubleAll )
end --shiftGems()
local function destroyGems()
    ...
    TimersST = timer.performWithDelay( 1000, shiftGems ) -- 3 sce
...
local function mission(event)
    ...
    if dataTable.mission == nil then
        ...
        mission.name = dataTable.mission[1].mission_name
    end
