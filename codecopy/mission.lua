...
local mission_exp = 0
local mission_pop = 0
local mission_score = 0
local imgMons = nil


local drop_pin_id = nil
local run_number = nil
local count_run_number = nil
local calculas_score = {}
...
local function enablePuzzleTouch(obj)
    ...
    if obj == 1 then
        BGdropPuzzle = display.newRoundedRect(0, _H*.4, _W, _H*.60,0)
        BGdropPuzzle.strokeWidth = 0
        BGdropPuzzle.anchorX = 0
        BGdropPuzzle.anchorY = 0
...
function onTouchGameoverFileScreen (  event,self )
...
local function showGameOver(text)
    ...
    -----------------
        ...
        local status = {}
        local touch_id = mission_id

        local path = system.pathForFile("datas.db", system.DocumentsDirectory)
        db = sqlite3.open( path )
        local updateplayMission ="update playMission set status=1 where station_id = '"..touch_id.."' and map_id = '"..NumberImg.."';"
        db:exec(updateplayMission)


        for iondata in db:nrows ("SELECT * FROM playMission WHERE map_id = '"..NumberImg.."' and station_id = '"..touch_id.."';") do
            data = data + 1
            datanext_station_id[data] = iondata.next_station_id

            for rowdata in db:nrows ("SELECT * FROM playMission WHERE map_id = '"..NumberImg.."' and station_id = '"..datanext_station_id[data].."';") do
                nm = nm +1

                playMission_id[nm] = tonumber(rowdata.playMission_id)
                station_id[nm] = tonumber(rowdata.station_id)
                next_station_id[nm] = rowdata.next_station_id
                status[nm] = tonumber(rowdata.status)

                local updateMission ="update playMission set status=0 where playMission_id = '"..playMission_id[nm].."' and status = '-1';"
                db:exec(updateMission)
            end
        end
        print("---------->>> oldscore = ",oldscore,"score = ",score)
        if NumberImg == 1 and touch_id == 3 then
            local nextmap_id = NumberImg + 1
            local updateplayMission ="update playMission set status=0 where station_id = '1' and map_id = '"..nextmap_id.."' and status = '-1';"
            db:exec(updateplayMission)



        elseif NumberImg == 2 and touch_id == 7 then
            local nextmap_id = NumberImg + 1
            local updateplayMission ="update playMission set status=0 where station_id = '1' and map_id = '"..nextmap_id.."' and status = '-1';"
            db:exec(updateplayMission)

            -- boss def
            local tablefill ="UPDATE aechiement SET score = '1' WHERE aechiement_element = '1' and aechiement_id = '8';"
            db:exec( tablefill )

        elseif NumberImg == 3 and touch_id == 6 then
            local nextmap_id = NumberImg + 1
            local updateplayMission ="update playMission set status=0 where station_id = '1' and map_id = '"..nextmap_id.."' and status = '-1';"
            db:exec(updateplayMission)

            -- boss def
            local tablefill ="UPDATE aechiement SET score = '1' WHERE aechiement_element = '1' and aechiement_id = '9';"
            db:exec( tablefill )

        elseif NumberImg == 4 and touch_id == 7 then
            local nextmap_id = NumberImg + 1
            local updateplayMission ="update playMission set status=0 where station_id = '1' and map_id = '"..nextmap_id.."' and status = '-1';"
            db:exec(updateplayMission)

            -- boss def
            local tablefill ="UPDATE aechiement SET score = '1' WHERE aechiement_element = '1' and aechiement_id = '10';"
            db:exec( tablefill )

        elseif NumberImg == 5 and touch_id == 5 then
            local nextmap_id = NumberImg + 1
            local updateplayMission ="update playMission set status=0 where station_id = '1' and map_id = '"..nextmap_id.."' and status = '-1';"
            db:exec(updateplayMission)

            -- boss def
            local tablefill ="UPDATE aechiement SET score = '1' WHERE aechiement_element = '1' and aechiement_id = '11';"
            db:exec( tablefill )

        elseif NumberImg == 6 and touch_id == 12 then
            local nextmap_id = NumberImg + 1
            local updateplayMission ="update playMission set status=0 where station_id = '1' and map_id = '"..nextmap_id.."' and status = '-1';"
            db:exec(updateplayMission)

            -- boss def
            local tablefill ="UPDATE aechiement SET score = '1' WHERE aechiement_element = '1' and aechiement_id = '12';"
            db:exec( tablefill )

        elseif NumberImg == 7 and touch_id == 5 then
            if oldscore > score then

                local tablefill ="UPDATE aechiement SET score = '"..score.."',scoreMax = '"..score.."' WHERE aechiement_element = '1' and aechiement_id = '13' and scoreMax > score;"
                db:exec( tablefill )
            else
                local tablefill ="UPDATE aechiement SET score = '"..score.."' WHERE aechiement_element = '1' and aechiement_id = '13';"
                db:exec( tablefill )
            end
    ...
    -----------------
        if score > oldscore then
            ...
            local tablefill ="UPDATE aechiementMap SET score = '"..score.."',scoreMax = '"..score.."' WHERE aechiementMap_id = '"..NumberImg.."';"
            db:exec( tablefill )
        else

            local tablefill ="UPDATE aechiementMap SET score = '"..score.."' WHERE aechiementMap_id = '"..NumberImg.."';"
            db:exec( tablefill )
        end

        for i=1,#countColor,1 do
            if aechiement[i].aechiement_score < aechiement[i].aechiement_scoreMax then
                countColor[i] =  tonumber(countColor[i] + aechiement[i].aechiement_score)

                local tablefill ="UPDATE aechiement SET score = '"..countColor[i].."' WHERE aechiement_element = '"..i.."' and aechiement_id = '"..i.."';"
                db:exec( tablefill )
            end
        end

        require( "info").Youwin(team_no,score,oldscore,mission_name,countColor,mission_exp,map_id)
        ...
        for i=1,#countColor,1 do
            if aechiement[i].aechiement_score < aechiement[i].aechiement_scoreMax then
                ...
                local tablefill ="UPDATE aechiement SET score = '0' WHERE aechiement_element = '"..i.."' and aechiement_id = '"..i.."';"
                db:exec( tablefill )
            end
        end
        ...
        print("call YouLoseMODE")
            require( "info").YouLoseMODE(option)
    end
end
local function gameTimerUpdate ()
    ...
    if gameTimer >= 0 then
        ...
        gameTimerText.text = string.format("time: ".. gameTimer.." sc."..pointStartheart)
...
local function newGemPop (i,j,num)
    ...
    local sizeGemYpop = sizeGemY /5
    local sizeGemXpop = sizeGemX/5
    --    newGem = display.newImageRect(picture[R],sizeGem,sizeGem)
    newGem = display.newImageRect(picturePop[R],sizeGemXpop,sizeGemYpop)
...
local function PopNumIconCharacter(color,countColor)
    ...
    if   TouchCount~=0 then
        ...
        if rowCharac then
            ...
            if color == "PINK" then
                myPink.alpha = 1
                local myCountDEF = math.ceil(((sumDEF*.025)+(countCombo*(1+(countCombo/10))) )/10)
                myPink.text = string.format("+"..myCountDEF)
                transitionStash.newTransition = transition.to(myPink, { time=700, alpha=0, xScale=1.5, yScale = 1.5} )
                gameTimer = gameTimer + myCountDEF
                pointStartheart =  pointStartheart +((512/gameTimerSET) * myCountDEF)

                hpPlayer =  math.ceil(gameTimer + myCountDEF)
...
local function calScore(noActive)
    local myCountDEF = 0
    ...
    for i=1,#countColor,1 do
        ...
        if oneTouch > 0  then
            if  i ~= 6 then
                local k = 0
                if noActive[i] == 4 then
                    k = 1.0
                elseif noActive[i] == 5 then
                    k = 1.1
                elseif noActive[i] == 6 then
                    k = 1.2
                elseif noActive[i] == 7 then
                    k = 1.3
                elseif noActive[i] == 8 then
                    k = 1.4
                elseif noActive[i] >= 9 then
                    k = 1.5
                end


                score = score + (mission_pop * k)
            else
                local k = 0
                if noActive[i] == 4 then
                    k = 1.0
                ...
                myPink.alpha = 1
                myPink.text = string.format("+"..k)
                transitionStash.newTransition = transition.to(myPink, { time=700, alpha=0, xScale=1.5, yScale = 1.5} )
                gameTimer = gameTimer + k
                pointStartheart =  pointStartheart +((512/gameTimerSET) * k)
    ...
    if countCombo == 1 then
        getCombo = 0
    elseif countCombo == 2 then
        getCombo = 0.2
    elseif countCombo == 3 then
        getCombo = 0.4
    elseif countCombo == 4 then
        getCombo = 0.6
    elseif countCombo == 5 then
        getCombo = 0.8
    elseif countCombo >= 6 then
        getCombo = 1
    end

    score = score + (mission_pop * getCombo )
        ...
        for j = 1, gemY, 1 do
            ...
            if gemsTable[i][j].isMarkedToDestroy  then
                ...
                if oneTouch > 0 then
                    ...
                    displayGetdate = gemsTable[i][j]
--                    transition.to( displayGetdate, { time=200, alpha=0.5, xScale=1.5, yScale = 1.5})
                    gemsTable[i][j] = newGemPop(i,j,num)
                    transition.to( gemsTable[i][j], { time=200, alpha=0.05, xScale=5, yScale = 5})
                end
                isGemTouchEnabled = false
...
local function cleanUpGems()
    ...
    for i = 1, gemX, 1 do
        for j = 1, gemY, 1 do
            ...
            if gemsTable[i][j].isMarkedToDestroy then
                print("i=",i,"j=",j)
                transitionStash.newTransition = transition.to( gemsTable[i][j], { time=100,alpha=0.2, xScale = 2, yScale = 2 } )
...
local function hideMonster()
    ...
    timer.performWithDelay ( 200,function()
        ...
        transition.to( displayShowMonster, { time=300, xScale = 0.7 ,yScale = 0.7 ,onComplete = function()

            transition.to( displayShowMonster, { time=300, x=displayShowMonster.x + _W ,onComplete = function()

                newGroup = displayShowMonster
                display.remove(newGroup)
                newGroup = nil

                if count_run_number < run_number then
                    count_run_number = count_run_number + 1
                    transition.to( displayShowMonster, { time=1, xScale = 1 ,yScale = 1})
                    showMonster()
...
function showMonster()
    ...
    textScore_request.text =  "/ "..mission_score * calculas_score[count_run_number]
end
...
function checkdoubleAll()
    ...
    if overFileMin >= limitCountGem then
        ...
        cleanUpGems()
        if score >= (mission_score*calculas_score[count_run_number]) and countTouch ~= 0 then
            enablePuzzleTouch(1)
            ...
            pauseTime(phase)
            timer.performWithDelay ( 500,hideMonster)
        end
...
function onGemTouch( self, event )	-- was pre-declared
    ...
    if event.phase == "began" and isGemTouchEnabled == true and gameTimer > 0 and pause.id == 1 then
        ...
        sizeLineStY, sizeLineStX = _H*.705, _W*.5
        ...
        local function gotoPage()
            if page == "yes" then    --yes
                if name == 1 then
                    ...
                    storyboard.gotoScene("mainMap","zoomInOutFade",200)
                end
...
local function Condition()
    ...
    local scoreRequest = mission_score * calculas_score[count_run_number]

    textScore_request = display.newText("/ "..scoreRequest,_W*.7,_H*.045,"CooperBlackMS",35)
    textScore_request.anchorX = 0
    textScore_request.anchorY = 1
    groupGameTop:insert (textScore_request )

end

local function loadMonster(map_id,mission_id)
    local path = system.pathForFile("datas.db", system.DocumentsDirectory)
    db = sqlite3.open( path )
    local rowaechiement = 0
    local mission_data = {}
    for Allrowaechiement in db:nrows ("SELECT * FROM mission where map_id = '"..map_id.."' and mission_id = '"..mission_id.."' ;") do
        rowaechiement = rowaechiement + 1
        mission_data[rowaechiement] = {}
        mission_data[rowaechiement].drop_pin_id = tonumber(Allrowaechiement.drop_pin_id)
        run_number = tonumber(Allrowaechiement.run_number)
        mission_exp = tonumber(Allrowaechiement.mission_exp)
        mission_pop = tonumber(Allrowaechiement.mission_pop)
        mission_score = tonumber(Allrowaechiement.mission_score)
        for Pindata in db:nrows ("SELECT * FROM pin WHERE pin_id = '"..mission_data[rowaechiement].drop_pin_id.."';") do
            imgMons = Pindata.pin_imgbig
        end
    end
end

function scene:createScene( event )
    calculas_score = {1,1.5,2}
    count_run_number = 1
    ...
    loadMonster(map_id,mission_id)

    for rowdata in db:nrows ("SELECT * FROM aechiementMap WHERE map_id = '"..map_id.."';") do
        oldscore = tonumber(rowdata.scoreMax)
    end

--    oldscore = 20
    score = 0
    gameTimerSET = 60 * 1 *(3)
    gameTimerOld = gameTimerSET
    ...
    for Allrowaechiement in db:nrows ("SELECT * FROM aechiement ;") do
        ...
        aechiement[rowaechiement].aechiement_score = tonumber(Allrowaechiement.score)
        aechiement[rowaechiement].aechiement_scoreMax = tonumber(Allrowaechiement.scoreMax)
        aechiement[rowaechiement].aechiement_element = Allrowaechiement.aechiement_element

    end

    for AllPindata in db:nrows ("SELECT * FROM team WHERE team_no = '"..team_no.."';") do
        row = row + 1
        picture[row] = {}
        picture[row].pin_id = AllPindata.pin_id

        for Pindata in db:nrows ("SELECT * FROM pin WHERE pin_id = '"..picture[row].pin_id.."';") do
            picture[row] = Pindata.pin_imgmini
        end
    end

    local group = self.view

    local mission_img = display.newImage( "img/mission/bg"..map_id..".jpg" )
