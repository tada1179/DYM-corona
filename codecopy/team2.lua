...
local function menuMode(event)
    if "ended" == event.phase   then
        print("team 2 event.target.id == ",event.target.id,"PinData[event.target.id].pin_id === ",PinData[event.target.id].pin_id)
        local option = {
            team_no = 2,
...
function newTEAM(USERID)
    ...
    for AllPindata in db:nrows ("SELECT * FROM team WHERE team_no = '2';") do
