...
function new(params)
	...
	function scrollView:addScrollBar(r,g,b,a)
		...
		local scrollColorG = g or 200
		local scrollColorB = b or 0
		local scrollColorA = a or 200



        local viewPortH = screenH - self.top - self.bottom
		local scrollH = (viewPortH*self.height/(self.height*2 - viewPortH) ) - 300

--		local scrollBar = display.newRoundedRect(viewableScreenW-50,200,5,scrollH,2)
--		scrollBar:setFillColor(scrollColorR, scrollColorG, scrollColorB, scrollColorA)


        local scrollBar = display.newImageRect("img/background/scroll_bar.png",15, scrollH )
        scrollBar:setReferencePoint( display.TopLeftReferencePoint )
        scrollBar.x, scrollBar.y = viewableScreenW-60, 0
